...
def download_dataset(dataset='cifar10',data_path='/data'):
    # load data
    param = {
        'cifar10': {'name': datasets.CIFAR10, 'size': 32, 'normalize': [[0.485, 0.456, 0.406], [0.229, 0.224, 0.225]]},
        'cifar100': {'name': datasets.CIFAR100, 'size': 32,
                     'normalize': [(0.507, 0.487, 0.441), (0.267, 0.256, 0.276)]},
        'mnist': {'name': datasets.MNIST, 'size': 32, 'normalize': [(0.5, 0.5, 0.5), (0.5, 0.5, 0.5)]},
        'tinyimagenet': {'name': datasets.ImageFolder, 'size': 224,
                         'normalize': [(0.4802, 0.4481, 0.3975), (0.2302, 0.2265, 0.2262)]}}
    data = param[dataset]

    if data['name'] == datasets.ImageFolder:
        raise NotImplementedError('ImageFolder dataset is not supported for downloading')
    else:
        _ = data['name'](root=data_path,
                                train=True,
                                download=True)
        _ = data['name'](root=data_path,
                     train=False,
