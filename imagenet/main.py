...
def main():
    ...
    if args.results_dir is None:
        os.makedirs(os.path.join("result", "stage1"), exist_ok=True)
        args.results_dir = os.path.join("result", "stage1")
    ...
    # launch stage 2 if needed
    if args.next_command:
        logging.info("Exit and submit the next job!")
        os.system(args.next_command.replace("+", " "))
...
def forward(data_loader, model, criterion, epoch=0, training=True, optimizer=None):
    ...
    if args.use_dali:
        for i, batch_data in enumerate(data_loader):
            ...
            #         loss_reg += module.tau[0] * torch.mean(0.5 - 0.5*(torch.cos( 2 * 30 * module.weight)))
